...
const SearchBar = () => {
  ...
  return (
    <div ref={plateWindow} className="relative z-30">
        ...
        <img src={xMark} alt="xMark" onClick={handleClearcClick} className={`${searchInput.input != "" ? 'absolute' : 'hidden'} cursor-pointer max-w-[14px] h-auto right-3 top-[15px] md:top-[11px]`}/>
        <form onSubmit={(e) => handleSubmitClick(e)}>
            <input value={searchInput.input} onChange={(e) => handleChanges(e)} placeholder='Type for search...' type="text" className='bg-darkLighter poppins rounded-none md:rounded-[5px]  md:focus:outline-offset-0 md:focus:outline-1 md:focus:outline-gray-400 focus:outline-none py-[10px] md:py-[5px] pl-10 pr-9 md:pl-[2.3rem] md:pr-9  md:max-w-[22rem] text-white font-light outline-none'/>
        </form>
