        ...
        <div className="flex w-full mt-3 gap-4">
            ...
            {isSuccessFilm && (
                <div className="flex flex-col ">
                    <p className="poppins text-white text-xs lg:text-base ">
                        {dataFilm.release_date.slice(0, 4)}{" "}
                        <span
                            className={`${dataFilm.runtime === 0 && "hidden"}`}
                        >
                            {" "}
                            • {runtimeCalc(dataFilm.runtime)}
                        </span>
                    </p>
                    <p className="poppins text-white text-xs lg:text-base mt-[5px] ">
                        {dataFilm.genres.slice(0, 1).map((genre) => {
                            return `${genre.name}`;
                        })}
                    </p>
                </div>
